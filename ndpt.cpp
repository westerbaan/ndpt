...
public:
    ...
    inline friend Vec<S,N>
    pwProd(const Vec<S,N>& v, const Vec<S,N>& w){
        Vec<S,N> ret;
        for (size_t i = 0; i < N; i++) ret.vals[i] = v.vals[i] * w.vals[i];
        return ret;
    }
...
public:
    ...
    intersect(const Ray<S,N> &ray, Hit<S,N>& hit) const override {
        ...
        if (a1 >= radius) return false;

        S a2 = (projCentre2 - centre2).length();
        if (a2 >= radius) return false;
        ...
        if (!leastPositiveIntersection<S>(
                    (d1 - b1) / speed1,
                    (d1 + b1) / speed1,
                    (d2 - b2) / speed2,
                    (d2 + b2) / speed2,
        ...
        if (hit.distance < eps<S>) return false;
